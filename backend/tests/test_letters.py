...
def test_create_letter_price_reflects_pdf_page_count(client):
    ...
    assert body["estimated_weight_g"] == 35
    assert body["weight_bracket"] == "21-100g"
    assert body["printing_cost"] == 0.75
    assert body["paper_cost"] == 0.25
...
def test_create_letter_with_acknowledgment_of_receipt_adds_fee(client):
    ...
    assert body["total_amount"] == 12.5


def test_create_letter_rejects_pdf_exceeding_max_weight(client):
    form = sample_letter_form()
    form.pop("message")
    # 399 pages simplex -> 399*5 + 10 = 2005g > 2000g maximum
    filename, filedata, content_type = make_pdf_file(page_count=399)
    response = client.post("/api/letters", data=form, files={"document": (filename, filedata, content_type)})
...
def test_create_letter_ignores_client_supplied_price_fields(client):
    ...
    response = client.post("/api/letters", data=form, files={"document": (filename, filedata, content_type)})
    assert response.status_code == 201
    assert response.json()["total_amount"] == 5.45
