...
- **Physical delivery (see [[delivery]])**: full status walk (assign → pickup → in-transit → out-for-delivery → deposit → recipient confirms), invalid transitions rejected, tracking number uniqueness, admin force-confirm fallback, webhook auth (valid/wrong/unconfigured secret, unknown tracking number). **Critical test**: no delivery-confirmation email of any kind before `DEPOSITED`, exactly one confirmation-request to the recipient on deposit, exactly one delivery-confirmed email to the sender only once the recipient actually confirms (or an admin force-confirms), no duplicate on a repeated/reused-token call (`test_delivery_routes.py::test_no_delivery_email_before_deposited_then_exactly_one_on_recipient_confirm`) — this is the single most important delivery test, don't skip or weaken it. **Also required**: a letter without `acknowledgment_of_receipt` must skip the recipient step entirely — deposit finalizes straight to `DELIVERED`, zero recipient emails, sender notified immediately (`test_delivery.py::test_deposit_without_ar_finalizes_immediately`, `test_delivery_routes.py::test_webhook_deposit_without_ar_finalizes_immediately`).
