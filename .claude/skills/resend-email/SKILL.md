...
- Active email types: `PAYMENT_CONFIRMATION` (sender, on payment success), `DELIVERY_CONFIRMATION_REQUEST` (recipient, once a delivery is `DEPOSITED` — the only email the recipient ever gets, and it never contains letter content), `DELIVERY_CONFIRMED_SENDER` (sender only, once the recipient confirms or an admin force-confirms — see [[delivery]]), `SYSTEM_ERROR` (admin/internal, only for critical failures). `RECIPIENT_NOTIFICATION`, `LETTER_OPENED`, `RECEIPT_CONFIRMED`, `DELIVERY_CONFIRMED_RECIPIENT` still exist as enum values (historical `email_events` rows reference them) but nothing sends them anymore — don't resurrect a "your letter was delivered" email to the recipient, they already know because they're the one who just confirmed it.
